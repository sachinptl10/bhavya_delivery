...
export const PlasmaBackground = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    // Colors: #160B0B, #C2402A, #F49D37, #FFE8C2
    const hexToRgb = (hex) => {
    ...
    gl.uniform3fv(locs.u_colors, colorsArray);
    ...
    // u_finish = vec4(3.04, 0.61, 0.016, 0.05) - Reduced grain to subtle level
    gl.uniform4f(locs.u_finish, 3.04, 0.61, 0.016, 0.05);
    // u_transform = vec4(7.0, 0.00, 0.16, 0.0)
    gl.uniform4f(locs.u_transform, 7.0, 0.00, 0.16, 0.0);
    // u_space = vec4(0.00, 0.00, pointer x, pointer y) -> cursor: off
    gl.uniform4f(locs.u_space, 0.0, 0.0, 0.0, 0.0);
    // u_cursor = vec4(presence, 4.0, 0.65, 0.30)
    gl.uniform4f(locs.u_cursor, 0.0, 4.0, 0.65, 0.30); // presence = 0
    ...
    const render = () => {
      if (isVisible) {
        ...
        gl.uniform4f(locs.u_scene, canvas.width, canvas.height, time * 0.86, 4.0);
